/* tslint:disable */
/* eslint-disable */
/**
 * Маркетинг и продвижение
 * <div class=\"description_important\">     Узнать больше о маркетинге и продвижении можно в <a href=\"https://seller.wildberries.ru/instructions/category/59d92bd3-6ea0-40f2-b762-ca8835d7d42e?goBackOption=prevRoute&categoryId=479385c6-de01-4b4d-ad4e-ed941e65582e\">справочном центре</a> </div>  Методы маркетинга и продвижения позволяют:   1. Получать информацию о кампаниях [продвижения](/openapi/promotion#tag/Kampanii) и [медиакампаниях](/openapi/promotion#tag/Media).   2. [Создавать](/openapi/promotion#tag/Sozdanie-kampanij) и [управлять](/openapi/promotion#tag/Upravlenie-kampaniyami) кампаниями.   3. Настраивать [параметры](/openapi/promotion#tag/Parametry-kampanij) кампаний — кластеры фраз, продвигаемые товары и так далее.   4. Управлять [финансами](/openapi/promotion#tag/Finansy) кампаний.   5. Выгружать [статистику](/openapi/promotion#tag/Statistika) кампаний продвижения и медиакампаний.   6. Работать с [календарём акций](/openapi/promotion#tag/Kalendar-akcij).  Данные синхронизируются с базой раз в 3 минуты. Статусы кампаний меняются раз в минуту. Ставки кампаний меняются раз в 30 секунд. 
 *
 * The version of the OpenAPI document: promotion
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface V0KeywordsStatistic
 */
export interface V0KeywordsStatistic {
    /**
     * Количество кликов
     * @type {number}
     * @memberof V0KeywordsStatistic
     */
    clicks: number;
    /**
     * CTR (Click-Through Rate) — показатель кликабельности
     * @type {number}
     * @memberof V0KeywordsStatistic
     */
    ctr: number;
    /**
     * Ключевая фраза
     * @type {string}
     * @memberof V0KeywordsStatistic
     */
    keyword: string;
    /**
     * Сумма затрат по ключевой фразе
     * @type {number}
     * @memberof V0KeywordsStatistic
     */
    sum: number;
    /**
     * Количество показов
     * @type {number}
     * @memberof V0KeywordsStatistic
     */
    views: number;
}

/**
 * Check if a given object implements the V0KeywordsStatistic interface.
 */
export function instanceOfV0KeywordsStatistic(value: object): value is V0KeywordsStatistic {
    if (!('clicks' in value) || value['clicks'] === undefined) return false;
    if (!('ctr' in value) || value['ctr'] === undefined) return false;
    if (!('keyword' in value) || value['keyword'] === undefined) return false;
    if (!('sum' in value) || value['sum'] === undefined) return false;
    if (!('views' in value) || value['views'] === undefined) return false;
    return true;
}

export function V0KeywordsStatisticFromJSON(json: any): V0KeywordsStatistic {
    return V0KeywordsStatisticFromJSONTyped(json, false);
}

export function V0KeywordsStatisticFromJSONTyped(json: any, ignoreDiscriminator: boolean): V0KeywordsStatistic {
    if (json == null) {
        return json;
    }
    return {
        
        'clicks': json['clicks'],
        'ctr': json['ctr'],
        'keyword': json['keyword'],
        'sum': json['sum'],
        'views': json['views'],
    };
}

export function V0KeywordsStatisticToJSON(json: any): V0KeywordsStatistic {
    return V0KeywordsStatisticToJSONTyped(json, false);
}

export function V0KeywordsStatisticToJSONTyped(value?: V0KeywordsStatistic | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'clicks': value['clicks'],
        'ctr': value['ctr'],
        'keyword': value['keyword'],
        'sum': value['sum'],
        'views': value['views'],
    };
}

