/* tslint:disable */
/* eslint-disable */
/**
 * Маркетинг и продвижение
 * <div class=\"description_important\">     Узнать больше о маркетинге и продвижении можно в <a href=\"https://seller.wildberries.ru/instructions/category/59d92bd3-6ea0-40f2-b762-ca8835d7d42e?goBackOption=prevRoute&categoryId=479385c6-de01-4b4d-ad4e-ed941e65582e\">справочном центре</a> </div>  Методы маркетинга и продвижения позволяют:   1. Получать информацию о кампаниях [продвижения](/openapi/promotion#tag/Kampanii) и [медиакампаниях](/openapi/promotion#tag/Media).   2. [Создавать](/openapi/promotion#tag/Sozdanie-kampanij) и [управлять](/openapi/promotion#tag/Upravlenie-kampaniyami) кампаниями.   3. Настраивать [параметры](/openapi/promotion#tag/Parametry-kampanij) кампаний — кластеры фраз, продвигаемые товары и так далее.   4. Управлять [финансами](/openapi/promotion#tag/Finansy) кампаний.   5. Выгружать [статистику](/openapi/promotion#tag/Statistika) кампаний продвижения и медиакампаний.   6. Работать с [календарём акций](/openapi/promotion#tag/Kalendar-akcij).  Данные синхронизируются с базой раз в 3 минуты. Статусы кампаний меняются раз в минуту. Ставки кампаний меняются раз в 30 секунд. 
 *
 * The version of the OpenAPI document: promotion
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { V0KeywordsStatistic } from './V0KeywordsStatistic';
import {
    V0KeywordsStatisticFromJSON,
    V0KeywordsStatisticFromJSONTyped,
    V0KeywordsStatisticToJSON,
    V0KeywordsStatisticToJSONTyped,
} from './V0KeywordsStatistic';

/**
 * 
 * @export
 * @interface V0KeywordsStatistics
 */
export interface V0KeywordsStatistics {
    /**
     * Дата
     * @type {Date}
     * @memberof V0KeywordsStatistics
     */
    date: Date;
    /**
     * 
     * @type {Array<V0KeywordsStatistic>}
     * @memberof V0KeywordsStatistics
     */
    stats: Array<V0KeywordsStatistic>;
}

/**
 * Check if a given object implements the V0KeywordsStatistics interface.
 */
export function instanceOfV0KeywordsStatistics(value: object): value is V0KeywordsStatistics {
    if (!('date' in value) || value['date'] === undefined) return false;
    if (!('stats' in value) || value['stats'] === undefined) return false;
    return true;
}

export function V0KeywordsStatisticsFromJSON(json: any): V0KeywordsStatistics {
    return V0KeywordsStatisticsFromJSONTyped(json, false);
}

export function V0KeywordsStatisticsFromJSONTyped(json: any, ignoreDiscriminator: boolean): V0KeywordsStatistics {
    if (json == null) {
        return json;
    }
    return {
        
        'date': (new Date(json['date'])),
        'stats': ((json['stats'] as Array<any>).map(V0KeywordsStatisticFromJSON)),
    };
}

export function V0KeywordsStatisticsToJSON(json: any): V0KeywordsStatistics {
    return V0KeywordsStatisticsToJSONTyped(json, false);
}

export function V0KeywordsStatisticsToJSONTyped(value?: V0KeywordsStatistics | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'date': value['date'].toISOString().substring(0,10),
        'stats': ((value['stats'] as Array<any>).map(V0KeywordsStatisticToJSON)),
    };
}

