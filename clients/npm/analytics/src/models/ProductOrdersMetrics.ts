/* tslint:disable */
/* eslint-disable */
/**
 * Аналитика и данные
 * <div class=\"description_important\">   Узнать больше об аналитике и данных можно в <a href=\"https://seller.wildberries.ru/instructions/ru/ru/subcategory/seller-analytics\">справочном центре</a> </div>  <div class=\"api-block\">  В разделе описаны методы получения:   1. [Воронки продаж](/openapi/analytics#tag/Voronka-prodazh)   2. [Поисковых запросов по вашим товарам](/openapi/analytics#tag/Poiskovye-zaprosy-po-vashim-tovaram)   3. [Истории остатков](/openapi/analytics#tag/Istoriya-ostatkov)   4. [Оценки товара](/openapi/analytics#tag/Ocenka-tovara)   5. [Аналитики продавца в формате CSV](/openapi/analytics#tag/Analitika-prodavca-CSV)  </div> 
 *
 * The version of the OpenAPI document: analytics
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface ProductOrdersMetrics
 */
export interface ProductOrdersMetrics {
    /**
     * Дата сбора статистики
     * @type {Date}
     * @memberof ProductOrdersMetrics
     */
    dt: Date;
    /**
     * Средняя позиция товара в результатах поиска
     * @type {number}
     * @memberof ProductOrdersMetrics
     */
    avgPosition: number;
    /**
     * Сколько раз товары из поиска заказали
     * @type {number}
     * @memberof ProductOrdersMetrics
     */
    orders: number;
}

/**
 * Check if a given object implements the ProductOrdersMetrics interface.
 */
export function instanceOfProductOrdersMetrics(value: object): value is ProductOrdersMetrics {
    if (!('dt' in value) || value['dt'] === undefined) return false;
    if (!('avgPosition' in value) || value['avgPosition'] === undefined) return false;
    if (!('orders' in value) || value['orders'] === undefined) return false;
    return true;
}

export function ProductOrdersMetricsFromJSON(json: any): ProductOrdersMetrics {
    return ProductOrdersMetricsFromJSONTyped(json, false);
}

export function ProductOrdersMetricsFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProductOrdersMetrics {
    if (json == null) {
        return json;
    }
    return {
        
        'dt': (new Date(json['dt'])),
        'avgPosition': json['avgPosition'],
        'orders': json['orders'],
    };
}

export function ProductOrdersMetricsToJSON(json: any): ProductOrdersMetrics {
    return ProductOrdersMetricsToJSONTyped(json, false);
}

export function ProductOrdersMetricsToJSONTyped(value?: ProductOrdersMetrics | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'dt': value['dt'].toISOString().substring(0,10),
        'avgPosition': value['avgPosition'],
        'orders': value['orders'],
    };
}

